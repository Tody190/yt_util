...
class Encryptor:
    def encrypt(self, src_str):
        ...
        for i in range(0, src_bytearray_len):
            ...
            salt_byte = random.randint(33, 93)
            src_byte_integer = src_byte // 33 + salt_byte
            src_byte_remainder = src_byte % 33 + salt_byte
    ...
    def decrypt(self, encrypted_str):
        ...
        for i in range(0, len(src_bytearray)):
            ...
            src_byte = (src_byte_integer - salt_byte)*33 + src_byte_remainder - salt_byte
...
class Authenticator():
    ...
    def write_encrypt_code(self, user_name, password):
        user_code = self.encryptor.encrypt(user_name)
        password_code = self.encryptor.encrypt(password)
        ...
        if os.path.exists(self.aut_file):
            with open(self.aut_file, "r") as f:
                ...
                try:
                    ...
                    user_name = self.encryptor.decrypt(code_split[0])
                    password = self.encryptor.decrypt(code_split[-1])
